...
export default {
  ...
  activate(state) {
    ...
    submitButton.addEventListener("click", e => {
      ...
      command = `ssh ${loginName}@${serverUrl} "mkdir ~/${workDir}/${studentId}: exit" `;
    ...
    atom.workspace.observeTextEditors(editor => {
      ...
      buffer.onDidSave(event => {
        if (platform == "win32") {
          command_windows = ` cd ${cwd_path} & where /Q git && if exist .git (git add --all && git commit -m 'commit_message' && scp -r .git ${loginName}@${serverUrl}:${workDir}/${studentId}/.git) else (git init & git add --all & git commit -m 'initial_commit' && scp .git ${loginName}@${serverUrl}:${workDir}/${studentId}/.git) `;

  ...
  activate(state) {
    ...
    atom.workspace.observeTextEditors(editor => {
      ...
      buffer.onDidSave((event) => {
        if ( platform == "win32") {
          command_windows = ` cd ${cwd_path} & where /Q git && if exist .git (git add --all && git commit -m 'commit_message' && robocopy .git ¥¥${loginName}@${serverUrl}:${workDir}/${studentId} /E /DCOPY ) else (git init & git add --all & git commit -m 'initial_commit' && rsync -auz -e ssh .git ${loginName}@${serverUrl}:${workDir}/${studentId}) `

...
export default {
  ...
  activate(state) {
    ...
    submitButton.addEventListener('click', (e) => {
      ...
      // establish ssh connection and create studentId dir?
      command = `
        ssh ${loginName}@${serverUrl} "mkdir ~/${workDir}/${studentId}"
      `
      ;
    ...
    atom.workspace.observeTextEditors(editor => {
      ...
      buffer.onDidSave((event) => {
        if ( platform == "win32") {
          command_windows = ` cd ${cwd_path} & where /Q git && if exist .git (git add --all && git commit -m 'commit_message' && scp -r .git ${loginName}@${serverUrl}:git¥${studentId} ) else (git init & git add --all & git commit -m 'initial_commit' && scp -r .git ${loginName}@${serverUrl}:git¥${studentId}) `
          ...
          //     scp -r ${studentId}.git ${loginName}@${serverUrl}:${workDir}/${studentId}
          //   fi
          // `
          command_mac = `
            cd ${cwd_path};
            if !(type git > /dev/null 2>&1); then
              ...
              git add --all;
              git commit -m "commit log" &&
              scp -r .git ${loginName}@${serverUrl}:${workDir}/${studentId}
            fi
          `
          ;
